...
export function Hero() {
  return <section className="hero navy"><Doodles light /><div className="container"><div className="hero-main">
    ...
    <div className="hero-number"><div className="round-icon"><Users size={23} /></div><strong><Counter value={schoolGallery.length} suffix="" /></strong><p>Moments in Our Gallery</p></div>
...
export function AboutSection({ extended = false }: { extended?: boolean }) {
  return <section className="section about-section"><div className="container"><div className="two-column about-grid">
    ...
    <Reveal direction="right" className="about-copy"><span className="eyebrow">A Little About Young Explorers Academy</span><h2>Learning Beyond the Classroom</h2><p>Young Explorers Academy is a skill-based learning centre for children in Baluwatar, Kathmandu. Creative experiences, outdoor discovery, and community events bring learning beyond the classroom.</p>
...
export function TourBanner() {
  return <section className="tour-banner wave-section adventure-banner navy"><Doodles light /><div className="container tour-banner-layout">
    <Reveal direction="left" className="tour-banner-content"><span className="eyebrow">A World Beyond the Classroom</span><h2>Their Next Adventure Starts Here</h2><p>Discover our camps, creative activities, and experiences beyond the classroom.</p><ButtonLink to="/admissions#enrollment">Enquire About Programmes</ButtonLink></Reveal>
    <Reveal direction="right" className="tour-banner-visual"><Photo className="tour-banner-image" src={assets.banner} alt="Young Explorers Academy children walking together along a sunlit forest trail" /></Reveal>
  </div></section>;
}
...
export function FAQSection({ full = false }: { full?: boolean }) {
  ...
  })}</div>)}</div><p className="faq-contact"><span className="round-icon"><Phone size={18} /></span> Ask us about the next adventure. <a href={academy.instagram} target="_blank" rel="noopener noreferrer">Message the Academy</a></p></div></section>;
}

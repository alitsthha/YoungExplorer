...
export function ClassesSection({ all = false }: { all?: boolean }) {
  ...
  return <section className="section cream wave-section classes-section"><Doodles /><div className="container"><SectionTitle eyebrow="Kids Activities and Programmes" title="Find Their Next Adventure in Kathmandu" description="Compare camps, outdoor learning, creative workshops, storytelling, and family activities, then ask about upcoming dates and availability." action={!all && <ButtonLink to="/classes">View All Programmes</ButtonLink>} />
...
const reasons = [
  { Icon: Users, title: 'Caring Guidance for Every Child', text: 'Patient teachers and a warm welcome help children feel ready to participate.' },
  { Icon: ShieldCheck, title: 'A Welcoming Place to Explore', text: 'Children can try new activities, build confidence, and learn at their own pace.' },
  { Icon: Lightbulb, title: 'Hands-On Learning Through Play', text: 'Open-ended experiences turn curiosity into practical skills and joyful discovery.' },
  { Icon: Sparkles, title: 'Outdoor Learning in Kathmandu', text: 'Nature-based activities invite children to observe, ask questions, and work together.' },
  { Icon: Heart, title: 'Creative Activities With Purpose', text: 'Art, stories, movement, and making give children different ways to express themselves.' },
  { Icon: School, title: 'A Community for Children and Families', text: 'Shared events and everyday connections help children feel they belong.' },
];
export function WhySection() {
  return <section className="section why-section"><div className="container"><SectionTitle centered eyebrow="Why Kathmandu Families Choose Us" title="Kids Activities That Help Children Explore and Grow" description="Young Explorers Academy combines caring guidance, outdoor discovery, creative activities, and community experiences for children in Baluwatar, Kathmandu. Families can ask about upcoming camps, workshops, and events to find the right fit." /><div className="why-grid"><div>{reasons.slice(0, 3).map(({ Icon, title, text }, i) => <Reveal direction="left" key={title} delay={i * 100} className="reason-card"><Icon /><div><h3>{title}</h3><p>{text}</p></div></Reveal>)}</div><Reveal direction="scale"><Photo src={assets.child} className="why-photo burst-photo" alt="A young explorer planting in the garden during an outdoor learning activity in Kathmandu" /></Reveal><div>{reasons.slice(3).map(({ Icon, title, text }, i) => <Reveal direction="right" key={title} delay={i * 100} className="reason-card"><Icon /><div><h3>{title}</h3><p>{text}</p></div></Reveal>)}</div></div></div></section>;
}
...
export function EnrollmentForm({ contact = false, initialProgram = '' }: { contact?: boolean; initialProgram?: string }) {
  ...
  return <form className="enrollment-form" onSubmit={submit} ref={formRef}>
    <span className="eyebrow">{contact ? 'Say Hello' : 'Check Availability'}</span><h2>{contact ? 'We’d Love to Hear From You' : 'Find Your Child’s Next Adventure'}</h2><p>{contact ? 'Have a question about our school? Tell us what is on your mind.' : 'Share your contact details and the programme you are considering. We will follow up with current dates, age guidance, and fees.'}</p>
    ...
    {!contact && <FormField id={`${uid}-contact`} label="Phone or Email"><input id={`${uid}-contact`} name="Phone or email" required minLength={6} placeholder="Your phone number or email" autoComplete="email" /></FormField>}
    {contact && <div className="form-row"><FormField id={`${uid}-email`} label="Email Address"><input id={`${uid}-email`} name="Email" type="email" required placeholder="Email address" autoComplete="email" /></FormField><FormField id={`${uid}-phone`} label="Phone Number"><input id={`${uid}-phone`} name="Phone" type="tel" required minLength={6} placeholder="Phone number" autoComplete="tel" /></FormField></div>}
    {!contact && <FormField id={`${uid}-program`} label="Programme of Interest"><select id={`${uid}-program`} name="Program" required defaultValue={initialProgram}><option value="" disabled>Choose a programme</option>{programs.map(p => <option key={p.slug} value={p.slug}>{p.title}</option>)}</select></FormField>}
    ...
    <button className="button" type="submit" disabled={status === 'sending'}>{status === 'sending' ? 'Sending…' : !endpoint ? 'Prepare Enquiry' : contact ? 'Send Message' : 'Check Availability'}</button>
